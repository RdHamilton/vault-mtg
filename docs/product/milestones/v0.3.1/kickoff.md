...
| R-6 | Clerk does not permit exact `http://localhost:51423/callback` as redirect URI on current plan tier | Low | High | Fallback: custom URL scheme (`vaultmtg://callback`) — requires new ticket; scope to separate spike if Clerk rejects exact-match localhost URI |
