...
- **Production deploy migration AccessDenied** - Fixed `infra/scripts/run-migrations.sh` calling `aws secretsmanager get-secret-value` directly. The EC2 instance role lacks that grant (S-03 least-privilege). The script now sources `DATABASE_URL` from `$BFF_ENV_FILE` (written at deploy time by `provision-db-url.sh` with inline credentials), eliminating the direct SM call and matching the credential model established by PRs #2539/#2540.
