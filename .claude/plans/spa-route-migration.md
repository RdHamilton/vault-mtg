...
|19 | **Remove Wails-era orphan root module** — `internal/` (~100,410 LOC), root `go.mod` + `go.sum` (mattn/go-sqlite3, modernc.org/sqlite, wails et al.), root-level Wails entry points if any. Pure deletion — the orphan isn't in `go.work` so no shipping binary embeds it, and the SQLite tooling chatter / IDE confusion goes away. | -100k | Pending | — |
