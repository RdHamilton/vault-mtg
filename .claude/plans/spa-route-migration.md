...
|24 | **Staging SSE Clerk auth (cross-domain cookie)** — `GET /api/v1/events` 401s on every reconnect on staging because the Dev Clerk `__session` cookie is scoped to `.clerk.accounts.dev` (the raw hosted host), and the browser doesn't send it on requests to `staging-api.vaultmtg.app`. Two paths scoped in the issue: **A.** CNAME `clerk-staging.vaultmtg.app` → Clerk Dev edge so cookies land in the `.vaultmtg.app` zone (production-correct, mirrors prod's `clerk.vaultmtg.app` setup — requires Clerk plan that supports custom domains on Dev instances). **B.** Extend `RequireClerkAuthForSSE` to also pull JWT from `?token=` query param, SPA appends Clerk token to SSE URL (~40 LOC + tests, no DNS / Clerk plan changes; trade-off: short-lived tokens land in nginx access logs unless `access_log off` for `/api/v1/events`). Match History / Stats / Decks / Collection are unaffected — SSE only carries live-draft and event-stream traffic. | ~40 | ⏳ **Open** — needs A vs B decision | Issue #1904 |

### Out-of-band fix (no PR — applied directly on staging during #23 investigation)

- **`daemon_events` schema drift on staging** — migration 069 (`ADD COLUMN sequence`) was registered as applied (schema_migrations at v76) but the column didn't exist. Root cause: table owner was `mtga_admin` (RDS master), not `vaultmtg_staging_app` (BFF migration user). `ADD COLUMN IF NOT EXISTS` requires `ALTER TABLE` permission which `GRANT ALL` doesn't grant in PostgreSQL — only ownership does. Same class of bug as the earlier `game_plays` ownership issue. Fixed via SSM RunShellScript using the RDS master credentials from Secrets Manager: `ALTER TABLE daemon_events OWNER TO vaultmtg_staging_app; ALTER TABLE daemon_events ADD COLUMN IF NOT EXISTS sequence BIGINT NOT NULL DEFAULT 0; CREATE INDEX IF NOT EXISTS idx_daemon_events_account_sequence ON daemon_events (account_id, sequence);`. Before fix: 0 rows in `daemon_events`, every `/api/v1/ingest/events` returned 202 with `ERROR persisting event ... column "sequence" of relation "daemon_events" does not exist` in BFF logs (silent drop). After fix: 140 rows on first reload, zero persistence errors. **Prod was not verified during this session** — connection to prod DB from the staging EC2 host failed with `password authentication failed`. Follow-up: confirm prod `daemon_events` has all expected columns and that ownership matches the migration user. Tracked going forward by `scripts/test/staging-auth-smoke.sh` test 9 (and the prod equivalent it should be parameterized to run against).
