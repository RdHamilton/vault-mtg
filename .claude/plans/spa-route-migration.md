...
|19 | **Remove Wails-era orphan root module** — `internal/`, `cmd/{mtga-companion,log-analyzer,apiserver}`, `examples/`, `benchmarks/`, root `go.mod` + `go.sum` (mattn/go-sqlite3, modernc.org/sqlite, kardianos/service, wails et al.). Drops the 6 legacy `cmd/mtga-companion-*` binary builds from `.github/workflows/release.yml`; updates `scripts/dev.sh` + `README.md` to point at per-service builds. | -109,994 | ⏳ **In progress** | `chore/phase2-pr19-orphan-removal` |
