...
Map the existing frontend landscape before writing code:
...
Leverage existing context before asking users. Focus on implementation specifics, validate assumptions, and request only mission-critical missing details.
...
- WebSocket integration for live updates (external integrations only — MTGA Companion uses SSE internally)
