...
q['agents']['daemon']['last_updated'] = datetime.datetime.utcnow().isoformat() + 'Z'
...
import json, datetime
with open('.claude/manager-queue.json') as f: q = json.load(f)
q['agents']['daemon'].update({'current_issue': None, 'current_pr': None, 'status': 'idle', 'last_updated': datetime.datetime.utcnow().isoformat() + 'Z'})
q['last_updated'] = datetime.datetime.utcnow().isoformat() + 'Z'
with open('.claude/manager-queue.json', 'w') as f: json.dump(q, f, indent=2)
