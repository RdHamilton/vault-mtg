...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_matches_account_id_timestamp
    ON matches (account_id, timestamp DESC);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_matches_account_id_format
    ON matches (account_id, format);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_matches_account_id_format_timestamp
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_draft_sessions_account_id_created_at
    ON draft_sessions (account_id, created_at DESC);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_draft_sessions_account_id_set_code
    ON draft_sessions (account_id, set_code);

-- player_stats: queries by account + date range, account + format + date
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_player_stats_account_id_date
    ON player_stats (account_id, date DESC);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_player_stats_account_id_format_date
    ON player_stats (account_id, format, date DESC);

-- decks: queries by account + modified_at (list/sort), account + format
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_decks_account_id_modified_at
    ON decks (account_id, modified_at DESC);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_decks_account_id_format
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_currency_history_account_id_timestamp_desc
    ON currency_history (account_id, timestamp DESC);

-- matchup_statistics: queries by account + format + archetype
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_matchup_stats_account_id_format
    ON matchup_statistics (account_id, format);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_matchup_stats_account_id_format_archetype
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_deck_perf_history_account_id_timestamp
    ON deck_performance_history (account_id, match_timestamp DESC);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_deck_perf_history_account_id_format
